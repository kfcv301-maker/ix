...
public class WebSocketServer extends TextWebSocketHandler {
            ...
            if (!Objects.equals(type, "1")) {
                ...
                if (node != null) {
                    ...
                    if (updateResult) {
                        ...
                        // 一次，避免用户必须打开节点页面再保存才能恢复。
                        syncNodeProtocolSettings(node);
                    } else {
    ...
    /**
     * 将面板保存的协议设置同步到刚连接的节点。
     * 仅写 agent 的本地开关，不会增删服务、转发、隧道或数据库记录。
     */
    private void syncNodeProtocolSettings(Node node) {
        final int http = node.getHttp() == null ? 0 : node.getHttp();
        final int tls = node.getTls() == null ? 0 : node.getTls();
        final int socks = node.getSocks() == null ? 0 : node.getSocks();
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                req.put("tls", tls);
                req.put("socks", socks);
                GostDto result = send_msg(nodeId, req, "SetProtocol");
                if (result == null || !Objects.equals(result.getMsg(), "OK")) {
                    log.warn("节点 {} 协议设置自动同步失败: {}", nodeId,
                            result == null ? "无响应" : result.getMsg());
                } else {
                    log.info("节点 {} 协议设置已自动同步", nodeId);
                }
            } catch (Exception e) {
                log.warn("节点 {} 协议设置自动同步异常: {}", nodeId, e.getMessage());
            }
